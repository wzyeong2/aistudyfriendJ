...
object StudyPrompt {
    ...
    private fun math(grade: Grade): String = buildString {
        ...
        appendLine("6. practice의 연습문제는 사진 속 문제와 '같은 형식·같은 길이·같은 난이도'로 2~3개 만들어줘.")
        appendLine("   - 원래 문제가 글로 된 문장제(서술형, 여러 줄)면 연습문제도 똑같이 문장제로 길게 써줘. 상황·이름·숫자만 바꾸고, 문장 길이는 비슷하게.")
        appendLine("   - 원래가 단순 계산식(예: 23+9)이면 계산식으로.")
        appendLine("   - 절대로 긴 문장제를 짧은 계산식으로 줄이지 마. 원래 문제의 형태를 그대로 따라가.")
    ...
    private fun english(grade: Grade): String = buildString {
        ...
        appendLine("5. practice의 연습문제는 사진 속 문제와 '같은 형식·같은 길이·같은 난이도'로 2~3개. 원래가 문장으로 된 문제면 연습문제도 문장형으로, 빈칸채우기면 빈칸채우기로 — 형태를 그대로 따라가. q는 영어 문제, a는 정답, hint는 한국어 힌트.")
